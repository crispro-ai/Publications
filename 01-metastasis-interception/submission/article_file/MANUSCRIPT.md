...
Metastasis accounts for over 90% of cancer-related mortality¹, yet therapeutic development remains focused on primary tumor targeting. Metastasis is an 8-step cascade (local invasion, intravasation, circulation survival, extravasation, micrometastasis formation, angiogenesis, and colonization), each governed by distinct genetic dependencies³⁴. Traditional "one-size-fits-all" therapeutic design cannot address this biological complexity.
...
To rank signal importance, we performed leave-one-out ablation. For each signal, we recomputed Target-Lock scores with that signal set to zero and measured AUROC drop per step. Signals with larger AUROC drops contribute more information. **Figure 6** shows Precision@K analysis (K=3, 5, 10) demonstrating perfect top-3 ranking (Precision@3 = 1.000) across all steps. **Figure 7** displays the ablation study results, quantifying the contribution of each signal (Functionality, Essentiality, Regulatory, Chromatin) to Target-Lock performance.
...
To assess robustness, we validated the **3-signal approach** (Functionality, Essentiality, Regulatory; excluding chromatin). Per-step AUROC was **0.989 ± 0.017**, AUPRC 0.962 ± 0.023, with Precision@3 = 1.000 (5000-bootstrap CIs, seed=42). This demonstrates that the core signals (85% combined weight) are sufficient for robust target selection, and chromatin (15% weight) is a minor component. **Figure 4** shows per-step ROC curves for the 4-signal validation, demonstrating consistent high performance across all 8 metastatic steps.

#### 4-Signal Validation (With Enformer Chromatin)

The **4-signal approach** (including Enformer chromatin) achieved per-step AUROC **0.988 ± 0.035**, AUPRC 0.962 ± 0.055, with Precision@3 = 1.000. All 8 steps showed significant enrichment (Fisher's exact p < 0.05, 8/8 with p < 0.001). Effect sizes were large (Cohen's d > 2.0 for Target-Lock scores). **Figure 2** displays the Target-Lock score heatmap across all 8 steps and 38 genes, showing clear enrichment of positive labels (genes mechanistically essential for each step). **Table 2** summarizes per-step performance metrics (AUROC, AUPRC, Precision@K) for the 4-signal approach.
...
- **Sequence-level prediction** (Evo2, trained on 9.3 trillion tokens across all domains of life)
- **Multi-modal biological signals** (functionality, essentiality, chromatin, regulatory)
- **Structural validation** (AlphaFold 3, 100% pass rate on 15 validated guides)
- **Stage-specific targeting** (8-step metastatic cascade, not just primary tumor)

To our knowledge, this combination of multi-modal biological scoring with structural pre-validation is unique among published CRISPR design platforms. The technical requirements (foundation model expertise, AlphaFold 3 API integration, RNA-DNA calibration knowledge) create substantial barriers to replication, but we emphasize that our contribution is methodological rather than proprietary—we provide open-source code and reproducible workflows to enable community adoption.
...
This stage-awareness enables targeting of vulnerabilities across all 8 metastatic steps (compared to primary tumor-only approaches) and enables rational therapeutic combinations—e.g., targeting BRAF (primary growth) + VEGFA (angiogenesis) + MET (colonization) for triple-hit metastasis prevention.
...
**RUO Disclaimer and Regulatory Path:** All results are Research Use Only. This platform is a hypothesis-generation and prioritization tool, not a clinical diagnostic. The path to FDA approval would require: (1) extensive wet-lab validation, (2) GLP-compliant preclinical studies, (3) IND-enabling toxicology, and (4) Phase I/II clinical trials. We estimate 3-5 years and $20-50M for a single therapeutic candidate to reach IND submission. However, the structural pre-screening provided by our platform reduces the risk of synthesis failures due to poor structural properties.

### Broader Implications: Foundation Models in Therapeutic Design

Our work demonstrates that genomic foundation models (Evo2) can be successfully integrated with structural biology tools (AlphaFold 3) to create end-to-end therapeutic design pipelines. This paradigm—sequence generation guided by biological context + structural validation before synthesis—is generalizable beyond CRISPR. Multi-modal validation (sequence + structure + function) provides multiple orthogonal filters to identify high-quality designs, compared to single-metric approaches. As foundation models proliferate, the bottleneck shifts from generation to validation—and structural pre-screening may become a critical filter. Additional applications and implications are discussed in Supplementary Discussion.
...
**Main Figures:**
- **Figure 1:** Multi-modal CRISPR design framework (see `figures/Kiani_Figure1.png` / `.svg`). Workflow diagram showing integration of Evo2, Enformer, and AlphaFold 3 for stage-specific CRISPR design across 8 metastatic steps.

- **Figure 2:** Target-Lock score heatmap (see `figures/F2_REAL_target_lock_heatmap.png` / `.svg`). 8 rows (metastatic steps) × 38 columns (genes). Color scale: 0-1 (darker = higher score). Positive labels (50 gene-step combinations) highlighted. Mean per-step AUROC: 0.988 ± 0.035, AUPRC: 0.962 ± 0.055.

- **Figure 3:** Structural validation metrics for 15 guide RNA:DNA complexes (see `figures/figure_6_structural_validation.png` / `.svg`). (A) pLDDT distribution: mean 65.6 ± 1.9, range 62.5-69.0. (B) iPTM distribution: mean 0.36 ± 0.015, range 0.33-0.38. (C) Structural confidence: mean 0.51 ± 0.02. All 15 guides: PASS (pLDDT ≥50, iPTM ≥0.30).

- **Figure 4:** Per-step ROC curves (see `figures/figure2a_per_step_roc.png` / `.svg`). 8 curves (one per metastatic step). X-axis: False Positive Rate, Y-axis: True Positive Rate. Mean AUROC: 0.988 ± 0.035 (5000-bootstrap 95% CI, seed=42).

- **Figure 5:** Step-specificity matrix (see `figures/figure2b_specificity_matrix.png` / `.svg`). 8×8 confusion matrix: rows = predicted step (highest Target-Lock score), columns = true step (ground truth). Diagonal dominance ratio per step shown. Fisher's exact p-values: all 8 steps p<0.05, 8/8 with p<0.001.

- **Figure 6:** Precision@K analysis (see `figures/figure2c_precision_at_k.png` / `.svg`). Bar chart: 8 steps × 3 bars (K=3, 5, 10). Y-axis: Precision. Precision@3 = 1.000 (8/8 steps), Precision@5 = 0.85-1.00, Precision@10 = 0.40-0.60.

- **Figure 7:** Ablation study (see `figures/figure2d_ablation.png` / `.svg`). Bar chart: 4 signals (Functionality, Essentiality, Regulatory, Chromatin) × 8 steps. Y-axis: AUROC change (leave-one-out). Mean changes: Functionality -0.042, Essentiality -0.038, Regulatory -0.025, Chromatin -0.013.

**Supplementary Figures:**
- **Figure S1:** Confounder analysis (see `figures/figure_s1_confounders.png` / `.svg`). Scatter plots: Target-Lock score vs gene length (ρ=0.12), GC content (ρ=0.18), exon count (ρ=0.25). All ρ<0.3, indicating minimal confounding.

- **Figure S2:** Calibration curves (see `figures/figure_s2_calibration_curves.png` / `.svg`). 8 panels (one per step). X-axis: Predicted score (5 quantile bins), Y-axis: Observed frequency. Diagonal line = perfect calibration. All steps show close alignment to diagonal.

- **Figure S3:** Effect sizes (see `figures/figure_s3_effect_sizes.png` / `.svg`). Bar chart: 4 signals × 8 steps. Y-axis: Cohen's d. All signals show d>0.8 (large effect) across all steps. Target-Lock composite: d>2.0 (8/8 steps).

### Tables
**Upload as: "Table" (one file per table)**

- **Table 2:** Performance metrics for Target-Lock validation across 8 metastatic steps (see `tables/table2_performance_metrics.csv` / `.tex`). Columns: Metastatic Step, n_samples, n_positive, auroc_mean (with 95% bootstrap CI), auprc_mean (with 95% bootstrap CI), Precision@3, Precision@5, Precision@10, Fisher's exact p-value, Cohen's d effect size.

- **Table S1:** 38 primary metastatic genes with clinical trial evidence (NCT IDs and PMIDs) (see `tables/table_s1_genes_nct_pmid.csv` / `.tex`). This table lists the 38 primary metastatic genes used for Target-Lock validation, including their associated clinical trial identifiers (NCT IDs) and PubMed IDs (PMIDs) supporting their role in metastasis.

- **Table S2:** Primary validation metrics (38 genes, 8 steps) (see `tables/table_s2_validation_metrics.csv` / `.tex`). Per-step AUROC and AUPRC for the full 38-gene validation set. Hold-out validation metrics (28 train / 10 test genes) are reported in the Results section.

- **Table S3:** Prospective validation genes (11 FDA-approved metastatic targets, 2024-2025) (see `supplementary_data/prospective_validation_genes_agent.csv`). Columns: gene, source, nct_id, fda_approval_date, pmid, priority_score, indication, notes. Contains 11 positive genes only. Negative controls (GAPDH, ACTB, TUBB3, ALB, INS, HBB, TP53, RB1) are in separate validation files (see `supplementary_data/prospective_validation_with_negatives_scores.csv`).

- **Table S4:** Structural validation details for 15 guide RNA:DNA complexes (see `tables/table_s4_structural_validation.csv` / `.tex`). Columns: Job ID, Metastatic Step, Target Gene, pLDDT, iPTM, Disorder (%), Clashes, Structural Confidence, Verdict. All 15 guides: Verdict=PASS, Disorder=0.0%, Clashes=No.
...
**Word Count**: ~4,900 words (target: 3,000-5,000 for Nature Biotechnology Articles)
